...
export interface MockTransportOptions {
	...
	/**
	 * If provided, transport starts in 'connecting' and emits 'open' after this delay (ms).
	 */
	openDelayMs?: number;
}
...
export class MockTransport implements Transport {
	...
	constructor(private readonly options: MockTransportOptions = {}) {
		const openDelay = options.openDelayMs ?? 0;
		if (openDelay > 0) {
	...
	send(data: string | ArrayBuffer | Uint8Array): void {
		...
		if (this.options.failSend) {
			this.dispatch(
				"error",
				new Error("MockTransport configured to fail send"),
			);
	...
	private dispatch<K extends keyof TransportEvents>(
		event: K,
		arg?: Parameters<TransportEvents[K]>[0],
	): void {
