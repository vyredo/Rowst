...
__export(src_exports, {
  ...
  WorkerPoolResolver: () => WorkerPoolResolver,
  generateUUID: () => generateUUID,
  isErrorMessage: () => isErrorMessage,
  isTransportClosed: () => isTransportClosed,
...
var WorkerPool = class {
  ...
  getOptimalWorkerCount() {
    ...
    try {
      const cpuList = typeof import_node_os.cpus === "function" ? (0, import_node_os.cpus)() : [];
      if (cpuList.length > 1) {
  ...
  resolveWorkerScript(scriptPath) {
    ...
    if (scriptPath) {
      if ((0, import_node_path.isAbsolute)(scriptPath)) {
        return scriptPath;
      }
      const candidateA = (0, import_node_path.join)(currentDir, scriptPath);
      if ((0, import_node_fs.existsSync)(candidateA)) return candidateA;
      const candidateB = (0, import_node_path.join)(process.cwd(), scriptPath);
      if ((0, import_node_fs.existsSync)(candidateB)) return candidateB;
    }
    const builtSibling = (0, import_node_path.join)(currentDir, "message-worker.js");
    if ((0, import_node_fs.existsSync)(builtSibling)) return builtSibling;
    const distWorker = (0, import_node_path.join)(process.cwd(), "dist", "workers", "message-worker.js");
    if ((0, import_node_fs.existsSync)(distWorker)) return distWorker;
    return builtSibling;
  }
};
